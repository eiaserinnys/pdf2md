...
class PdfElementManager:
    ...
    def add_element(self, mode, key, index, safe, visible, can_be_split, x1, y1, x2, y2, c1, c2):
        ...
        if safe and visible:
            ...
            if c1 is not None:
                text_id = self.canvas.create_text(x1 + 2 - width/2, y1 + 2 - width/2, text="concat" if c1 is 1 else "join", font=("TkDefaultFont", 7), anchor='nw', fill='white')
                text_bg = self.canvas.create_rectangle(self.canvas.bbox(text_id), fill=fill)
                self.canvas.tag_lower(text_bg, text_id)

            if c2 is not None:
                text_id = self.canvas.create_text(x2 - 2 - width/2, y2 - 2 - width/2, text="concat" if c2 is 1 else "join", font=("TkDefaultFont", 7), anchor='se', fill='white')
                text_bg = self.canvas.create_rectangle(self.canvas.bbox(text_id), fill=fill)
                self.canvas.tag_lower(text_bg, text_id)
